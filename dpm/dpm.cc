...
DPM::DPM(Data* data)
        : da(data), cl(*da),
          ...
          lambda(0.1),
          // distributions
          pd_tfbs_alpha(gsl_matrix_alloc(DPM::TFBS_LENGTH, DPM::NUCLEOTIDES)),
          pd_bg_alpha(gsl_matrix_alloc(DPM::BG_LENGTH, DPM::NUCLEOTIDES))
{
        ...
        for (int i = 0; i < DPM::BG_LENGTH; i++) {
                for (int j = 0; j < DPM::NUCLEOTIDES; j++) {
                        gsl_matrix_set(pd_bg_alpha, i, j, DPM::TFBS_LENGTH);
                }
        }
...
void
DPM::release_block(Data::element& element) {
...
void
DPM::assign_block(Data::element& element, Cluster::cluster_tag_t c) {
        if (c == 0) {
...
DPM::sample(Data::element& element) {
        ...
        char buf[DPM::TFBS_LENGTH];
        da->get_nucleotide(element, DPM::TFBS_LENGTH, buf);
