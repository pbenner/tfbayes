        ...
        const count_t * obs)
{
	...
	do {
		...
		if (last_move != MV_UP)
		{
			...
			ii = 0 ;
			do {
				printf("[pt_ln_marginal_likelihood] Node: %lu - Computing score\n", node_ids[ii]) ;
				tree->scores[node_ids[ii]] = _get_ln_score(tree,node_ids[ii]) ;
				ii++ ;
			}
			while ( node_ids[ii] != 0 );
...
double _dirichlet_eval(
        ...
        short alphabet_size)
{
	double sum_params, sum_counts, result;
	int ii ;
	...
	for ( ii = 0 ; ii < alphabet_size ; ii++ )
	{
		...
		printf("%f,%lu\t", dirichlet_params[ii], obs[ii]) ;
	}
